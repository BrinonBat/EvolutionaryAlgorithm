...
def createCSV(selection_name,crossover_name,insertion_name,cross_proba,mutate_proba,seeds):
    ...
    with open("V3.1.0/results/fit_"+csv_name+".csv", 'a', newline='') as csv_file:
...
def show(csv_name):
    #gather the datas
    with open("V3.1.0/results/fit_"+csv_name+".csv", 'r+', newline='') as csv_file:
    ...
    plt.grid()
    plt.legend()
    plt.savefig("V3.1.0/results/"+csv_name+".png")
    plt.clf()

def createFunctionTracer(li_mutations,csv_name):
    with open("V3.1.0/results/func_"+csv_name+".csv", 'a', newline='') as csv_file:
        csv_writer = csv.writer(csv_file, delimiter=';',dialect='unix',quoting=csv.QUOTE_NONE)
...
def generateWheelsGraph(wheels_records,max_gen):
    ...
    plt.grid()
    plt.legend()
    plt.savefig("V3.1.0/results/"+name+".png")
